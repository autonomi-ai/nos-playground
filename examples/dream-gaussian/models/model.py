...
class DreamGaussian:
    ...
    def preprocess(self, img: np.ndarray):
        ...
        input_img_torch = torch.nn.functional.interpolate(input_img_torch, (self.ref_size, self.ref_size), mode="bilinear", align_corners=False)

        input_alpha_torch = torch.from_numpy(input_alpha).permute(2, 0, 1).unsqueeze(0).to(self.device)
        input_alpha_torch = torch.nn.functional.interpolate(input_alpha_torch, (self.ref_size, self.ref_size), mode="bilinear", align_corners=False)
